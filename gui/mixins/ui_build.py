...
class UiBuildMixin:
    ...
    def _build_load_section(self: "BatchRunnerGUI", parent):
        ...
        col_delim = tk.Frame(top_row, bg=C["mantle"])
        col_delim.pack(side="left", fill="x", expand=True, padx=(8, 0))
        _dl_lbl = tk.Label(col_delim, text="Delimiter", font=FONTS["label"],
                 bg=C["mantle"], fg=C["subtext"])
        _dl_lbl.pack(anchor="w")
        Tooltip(_dl_lbl, TOOLTIPS["delimiter"])
        self._delimiter_combo = ttk.Combobox(
            col_delim, textvariable=self._ov_delimiter,
            values=["auto", ",", "\\t", "|", ";"],
